...
var buildGeminiRequest = (title, content, apiKey) => (sendRequester, config) => {
...
function deduplicateThemes(themes) {
  const strings = themes.filter((t) => typeof t === "string").map((t) => t.toLowerCase().trim());
...
function onHttpTrigger(runtime2, payload) {
  ...
  try {
    ...
    runtime2.log(`[Step 1] Processing story: ${input.storyId}`);
    ...
    try {
      ...
      runtime2.log(`[Step 2] Gemini error: ${msg}`);
      throw err;
    }
