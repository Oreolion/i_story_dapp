  ...
  supabaseClient: {
    ...
    storage: {
      from: () => ({
        ...
        getPublicUrl: vi.fn().mockReturnValue({ data: { publicUrl: 'https://mock-storage.com/audio.webm' } }),
      }),
    },
  },
}));
...
describe('RecordPage', () => {
  ...
  it('renders the main recording interface', () => {
    ...
    expect(screen.getByPlaceholderText('Give your story a title...')).toBeInTheDocument();
  });
  ...
  it('handles text input for title and transcription', () => {
    ...
    fireEvent.change(transcriptionArea, { target: { value: 'This is a test entry.' } });

    expect((titleInput as HTMLInputElement).value).toBe('My Day');
    expect((transcriptionArea as HTMLTextAreaElement).value).toBe('This is a test entry.');
  });
