  ...
  useEffect(() => {
    ...
    const fetchBook = async () => {
      try {
        ...
        // 2. Fetch Author Details (Manual fetch to avoid join errors)
        let authorProfile = { name: "Unknown", wallet_address: "", avatar: "" };
        if (bookData.author_id) {
            const { data: authorData } = await supabase
                ...
                .single();
            if (authorData) authorProfile = authorData;
        }
  ...
  if (!book) {
    return (
        <div className="min-h-screen flex flex-col items-center justify-center space-y-4">
            ...
            <Button variant="outline" onClick={() => router.back()}>Return to Library</Button>
        </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="bg-gradient-to-br from-indigo-900 via-purple-900 to-slate-900 rounded-3xl p-8 md:p-12 text-white shadow-2xl relative overflow-hidden min-h-[300px] flex flex-col justify-center">
                <div className="relative z-10">
                    <Badge className="mb-6 bg-white/20 hover:bg-white/30 text-white border-0 backdrop-blur-md px-3 py-1">
                        NFT Collection
                    </Badge>
                    <h1 className="text-4xl md:text-6xl font-bold mb-6 leading-tight tracking-tight">{book.title}</h1>
                    <p className="text-xl text-indigo-200 max-w-2xl font-light leading-relaxed">{book.description}</p>
                    ...
                    <div className="flex flex-wrap items-center gap-8 mt-10 pt-8 border-t border-white/10">
                        <div className="flex items-center gap-3">
                            <Avatar className="border-2 border-white/50 h-10 w-10">
                                ...
                                <AvatarFallback className="text-black">{book.author?.name?.charAt(0) || "A"}</AvatarFallback>
                            </Avatar>
                            <div className="text-sm">
                                <p className="opacity-60 text-xs uppercase tracking-wider">Author</p>
                                <p className="font-semibold">{book.author?.name || "Anonymous"}</p>
                            </div>
                        </div>
                        <div className="text-sm">
                             <p className="opacity-60 text-xs uppercase tracking-wider">Published</p>
                        ...
                        <div className="text-sm">
                             <p className="opacity-60 text-xs uppercase tracking-wider">Chapters</p>
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-2 text-2xl font-bold text-gray-900 dark:text-white px-2">
                                ...
                                <CardContent className="p-6">
                                    <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-3">
                                                <span className="text-sm font-mono text-gray-400">#{String(index + 1).padStart(2, '0')}</span>
                                                <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-100 group-hover:text-purple-600 transition-colors">
                                                    {chapter.title}
                                                </h3>
                                                {chapter.mood && (
                                        ...
                                        <div className="pl-10 md:pl-0 text-xs text-gray-400 whitespace-nowrap flex items-center">
            ...
            {chapters.length === 0 && (
                <div className="text-center py-16 text-gray-500 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-dashed">
