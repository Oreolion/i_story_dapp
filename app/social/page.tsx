...
export default function SocialPage() {
  ...
  const [unlockedStories, setUnlockedStories] = useState<Set<number>>(new Set());
  ...
  useEffect(() => {
        ...
        const updatedStories = await Promise.all(
          stories.map(async (story) => {
            try {
              ...
              const paywall = await storybookNFT.read.getPaywall(BigInt(story.id));
              return {
                ...story,
                likes: Number(likes || 0n),
                paywallAmount: Number(paywall || 0n),
              };
            } catch (storyError) {
              console.warn(`No on-chain data for story ${story.id} (using mock):`, storyError);
              return story; // Per-story mock fallback
            }
          })
        );
        setStories(updatedStories);
        toast.success('Loaded from blockchain (some may use mocks)', { duration: 2000 }); // Short toast
      } catch (error) {
        console.error('On-chain fetch failed:', error);
        toast.error('Blockchain load failed—switched to offline mode', { duration: 2000 });
        setUseOnChainData(false);
    ...
    fetchOnChainData();
  }, [useOnChainData, likeSystem, storybookNFT, address, stories.length]); // Deps to prevent repeat
  ...
  const handleLike = async (storyId: number) => {
    ...
    try {
      await likeSystem.write.likeStory(BigInt(storyId), address as `0x${string}`);
      toast.success('Story liked! +1 $STORY token earned');
      if (useOnChainData) {
        const likes = await likeSystem.read.getLikes(BigInt(storyId));
        setStories(prev => prev.map(story => story.id === storyId ? { ...story, likes: Number(likes || 0n) } : story));
      }
    } catch (error) {
      setStories(prev => prev.map(story => {
        if (story.id === storyId) {
  ...
  const handleUnlock = async (storyId: number) => {
    ...
    try {
      await iStoryToken.write.payPaywall(story.author.username as `0x${string}`, story.paywallAmount, BigInt(storyId));
      ...
      if (useOnChainData && storybookNFT) {
        ...
        setStories(prev => prev.map(s => s.id === storyId ? { ...s, paywallAmount: Number(paywall || 0n) } : s));
      }
    } catch (error) {
  ...
  if (isLoading) {
    return <div className="text-center py-8">Loading stories from blockchain...</div>;
  }
        ...
        <CardContent className="pt-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              { 
                label: 'Your $ISTORY', 
                value: iStoryToken?.balance ? `${Number(iStoryToken.balance / BigInt(1e18)).toFixed(2)}` : '0', 
                icon: Star, 
